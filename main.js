...
this.emergency=new EmergencySystem(this.scene,defs,this);this.dayNight=new DayNightSystem(this.scene);this.weather=new WeatherSystem(this.scene);const locations=(await (await fetch("./data/locations.json")).json()).locations;this.map=new MapSystem(this,locations);this.streaming=new WorldStreaming(this);this.economy=new EconomySystem(this);this.settingsUI=new Settings(this);this.applyQuality(this.settings.quality==="Medium"?this.autoQuality:this.settings.quality);this.performance=new PerformanceManager(this);set(100,"Phase 9 optimized renderer ready");document.getElementById("loading").classList.add("hidden");this.ui.setMission(this.missions.current)}
